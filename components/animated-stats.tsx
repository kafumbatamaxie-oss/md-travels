...
function StatItem({ end, suffix = "", label }: StatItemProps) {
  ...
  return (
    <div ref={ref} className="text-center p-6 bg-white backdrop-blur-sm rounded-2xl border border-white/10">
      <div className="text-4xl md:text-5xl font-bold text-secondary mb-2">
        {count}
        {suffix}
      </div>
      <p className="text-sm uppercase tracking-widest text-primary/50 font-bold">{label}</p>
    </div>
  )
}
