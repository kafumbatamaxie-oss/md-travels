...
export async function generateCorollaInvoicePDF(data: any) {
  ...
  const rate =
    data.package === "half" ? HALF_DAY : FULL_DAY;

  const total = rate * data.days;
  ...
  const rows = [
    {
      ...
      destination: `${data.package === "half" ? "Half Day" : "Full Day"} Transport`,
      rate: rate,
    },
    {
      date: new Date(data.pickupDate).toLocaleDateString(),
      ...
      destination: `${data.pickupLocation} → ${data.dropoffLocation}`,
      rate: 0,
    },
  ];
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.metaSection}>
          ...
          <View style={styles.clientBox}>
            ...
            <Text>Vehicle: Toyota Corolla 4-Seater</Text>
          ...
          {rows.map((row, i) => (
            <View key={i} style={styles.tr}>
              ...
              <Text style={styles.tdRight}>
                {row.rate === 0 ? "-" : `R ${row.rate.toFixed(2)}`}
              </Text>
            </View>
          ))}
        ...
        <View style={styles.terms}>
          ...
          <Text>• Routes subject to travel timing.</Text>
