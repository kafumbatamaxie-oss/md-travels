...
export async function generateRomionInvoicePDF(data: any) {
  const HALF_DAY = 2100;
  const FULL_DAY = 4200;
  const total = data.days === 1 ? HALF_DAY : FULL_DAY * data.days;
  const deposit = total / 2;

  const invoiceDate = new Date().toLocaleDateString();
  const refNumber = "MD-" + Math.random().toString(36).slice(2, 8).toUpperCase();

  const rows = [
    {
      date: data.pickupDate,
      no: "1",
      vehicle: "Toyota Romion 7-Seater",
      destination: `Pickup: ${data.pickupTime} → Dropoff: ${data.dropoffTime}`,
      rate: total,
    },
  ];
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.metaSection}>
          <View style={styles.clientBox}>
            ...
            <Text>Vehicle: Toyota Romion 7-Seater</Text>
        ...
        <View style={styles.table}>
          ...
          <View style={styles.depositRow}>
            ...
            <Text style={styles.totalText}>R {deposit.toFixed(2)}</Text>
          </View>
        </View>
        ...
        <View style={styles.terms}>
          ...
          <Text>• Vehicle fully insured and professionally driven.</Text>
...
const styles = StyleSheet.create({
  ...
  depositRow: { paddingHorizontal: 6, paddingBottom: 6, flexDirection: "row", justifyContent: "space-between" },
